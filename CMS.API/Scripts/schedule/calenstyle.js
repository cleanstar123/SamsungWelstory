...
$.CalenStyle = $.CalenStyle ||
{
	...
	defaults: //Plugin Defaults
	{
		...
		language: "en",
		veryShortDayNames: ["일", "월", "화", "수", "목", "금", "토"],
        ...
        inputTZOffset: "+09:00",
        tz: "Asia/Seoul", // For Google Calendar
        outputTZOffset: "+09:00", // +05:30
        ...
        agendaViewDuration: "CustomDays", // ["Month", "Week", "CustomDays"]
		daysInAgendaView: 7,
        agendaViewTheme: "Timeline1", // ["Timeline1", "Timeline2", "Timeline3"]
		showDaysWithNoEventsInAgendaView: true,

		fixedHeightOfWeekPlannerViewCells: true,

		quickAgendaViewDuration: "Week", // ["Week", "CustomDays"]
		daysInQuickAgendaView: 5,
		navigateOneDayInQuickAgendaView: true,

		taskPlannerViewDuration: "Week", // ["Week", "CustomDays"]
		daysInTaskPlannerView: 5,
		fixedHeightOfTaskPlannerView: true,
		navigateOneDayInTaskPlannerView: true,
        ...
        datasetModificationRule: "ReplaceAll", //["Default", "ReplaceAll", "ReplaceSpecified"]
        ...
        //eventColorsArray: ["6ad9e0", "6ab5e0", "a5b6f9", "bf91de", "ed6d97", "faa476", "edc456", "b1c74c", "3ee192", "25bbad"],
...
CalenStyle.prototype = {
	...
	getDateInFormat: function(oInput, sFormat, b24Hour, bIsLocalized)
	{
        ...
        else if (sFormat === "DDDD yyyy MMMM dd") {
            if ((to.setting.formatDates["DDDD yyyy MMMM dd "] !== undefined))
                oDateInFormat = to.setting.formatDates["DDDD yyyy MMMM dd"].call(to, oInput.iDate) 
            else
                oDateInFormat = "<b>" + to.getDateInFormat({ "iDate": oInput.iDate }, "DDDD", b24Hour, bIsLocalized) + " - " + to.getDateInFormat({ "iDate": oInput.iDate }, "yyyy", b24Hour, bIsLocalized) + '년 ' + to.getDateInFormat({ "iDate": oInput.iDate }, "MMMM", b24Hour, bIsLocalized) + " " + to.getDateInFormat({ "iDate": oInput.iDate }, "dd", b24Hour, bIsLocalized) + "일</b>";
        }
...
CalenStyle.prototype = $.extend(CalenStyle.prototype, {
	...
	_setMonthStrings: function()
	{
		...
		sUnderlineClass = bDatePickerView ? "cContHeaderLabelUnderline" : "", sClickableClass = bDatePickerView ? "clickableLink" : "";
        ...
        sHeaderViewLabel += "<span class='cContHeaderLabelMonth " + sClickableClass + "'><span class='" + sUnderlineClass + "'><b>" + to.getDateInFormat({ "iDate": { M: to.setting.selectedDate.getMonth() } }, "MMMM", false, true) + "</b></span></span>";
...
CalenStyle.prototype = $.extend(CalenStyle.prototype, {
	...
	_setDateStringsInDetailView: function()
	{
        ...
        if ($.cf.compareStrings(to.setting.visibleView, "DayView") || ($.cf.compareStrings(to.setting.visibleView, "CustomView") && to.setting.daysInCustomView === 1)) {
            sHeaderViewLabel = to.getNumberStringInFormat(oDVEnd.y, 0, true) + "년 " + "<b>" + to.getDateInFormat({ "iDate": oDVEnd }, "MMMM", false, true) + " " + to.getNumberStringInFormat(oDVEnd.d, 0, true) + "일 " + to.getDateInFormat({ "iDate": oDVStart }, "DDDD", false, true) + "</b>  ";
        }
		else
		{
			if(oDVStart.y === oDVEnd.y)
            {
				//if(oDVStart.M === oDVEnd.M)
                    //sHeaderViewLabel = to.getNumberStringInFormat(oDVEnd.y, 0, true) + "년 " + "<b>" + to.getDateInFormat({"iDate": oDVStart}, "MMM", false, true) + " " + to.getNumberStringInFormat(oDVStart.d, 0, true) + "</b>  -  <b>" + to.getNumberStringInFormat(oDVEnd.d, 0, true) + "</b>  ";
				//else
                    sHeaderViewLabel = to.getNumberStringInFormat(oDVEnd.y, 0, true) + "년 " + "<b>" + to.getDateInFormat({"iDate": oDVStart}, "MMM", false, true) + "  " + to.getNumberStringInFormat(oDVStart.d, 0, true) + "일 </b>  -  <b>" + to.getDateInFormat({"iDate": oDVEnd}, "MMM", false, true) + " " + to.getNumberStringInFormat(oDVEnd.d, 0, true) + "일 </b>  ";
			}
			else
...
CalenStyle.prototype = $.extend(CalenStyle.prototype, {
	...
	_setDateStringsInHeaderOfQuickAgendaView: function()
	{
		...
		if($.cf.compareStrings(to.setting.quickAgendaViewDuration, "CustomDays") && to.setting.daysInQuickAgendaView === 1)
            sHeaderViewLabel = to.getNumberStringInFormat(oDVEnd.y, 0, true) + "<b>" + to.getDateInFormat({"iDate": oDVStart}, "DDDD", false, true) + " " + to.getDateInFormat({"iDate": oDVEnd}, "MMMM", false, true) + " " + to.getNumberStringInFormat(oDVEnd.d, 0, true) + "</b>  ";
		else
		{
			if(oDVStart.y === oDVEnd.y)
			{
				if(oDVStart.M === oDVEnd.M)
                    sHeaderViewLabel = to.getNumberStringInFormat(oDVEnd.y, 0, true) + "<b>" + to.getDateInFormat({"iDate": oDVStart}, "MMM", false, true) + " " + to.getNumberStringInFormat(oDVStart.d, 0, true) + "</b>  -  <b>" + to.getNumberStringInFormat(oDVEnd.d, 0, true) + "</b>  ";
				else
			...
			else
			{
				if(iNumOfDays > 0)
				{
					if(to.compareDates(to.setting.selectedDate, to.tv.dAVDt[0]) >= 0 || to.compareDates(to.setting.selectedDate, to.tv.dAVDt[iNumOfDays - 1]) <= 0)
                        sSelectedDateElemId = "#Date-" + to.getDateInFormat({ "date": to.setting.selectedDate }, "yyyy-MM-dd", false, true);
				}
				...
				if($.cf.compareStrings(to.setting.agendaViewTheme, "Timeline1"))
				{
					...
					for(iTempIndex = 0; iTempIndex < iNumOfDays; iTempIndex++)
					{
                        ...
                        sFullDate = to.getDateInFormat({ "date": dTempViewStartDate }, "yyyy-MM-dd", false, true); /* 날짜 수정 */
						...
						{
                            $(to.elem).find(".cagvTable").append("<tr><td colspan='" + iColspan + "'><div id='" + sDateId + "' class='" + sDateClass + "'>" + to.getDateInFormat({ "date": dTempViewStartDate }, "DDDD yyyy MMMM dd", true, true) + "</div></td></tr>");
							...
							for(iEventIndex = 0; iEventIndex < oAEventsForView.length; iEventIndex++)
							{
								...
								sTemplate += "<div class='cagvEventDesc'>" + sDesc + "</div>";  /* 수정 */
				...
				else if($.cf.compareStrings(to.setting.agendaViewTheme, "Timeline2"))
				{
					...
					for(iTempIndex = 0; iTempIndex < iNumOfDays; iTempIndex++)
					{
                        ...
                        sFullDate = to.getDateInFormat({ "date": dTempViewStartDate }, "yyyy-MM-dd", false, true);   /* 수정 dd-MM-yyyy */
						sDateId = "Date-" + sFullDate;
				...
				else if($.cf.compareStrings(to.setting.agendaViewTheme, "Timeline3"))
				{
					...
					for(iTempIndex = 0; iTempIndex < iNumOfDays; iTempIndex++)
					{
                        ...
                        sFullDate = to.getDateInFormat({ "date": dTempViewStartDate }, "yyyy-MM-dd", false, true);
						sDateId = "Date-" +  sFullDate;
						sDateClass = bIsToday ? "cagvDate cagvDateToday" : "cagvDate";
				...
				setTimeout(function()
				{
                    ...
                    {
                        console.log(sSelectedDateElemId);
		...
		else
		{
			...
			if(to.tv.iNoVDay > 1)
			{
				if(oAGVStart.y === oAGVEnd.y)
                    sHeaderViewLabel = to.getNumberStringInFormat(oAGVEnd.y, 0, true) + ' ' + to.getDateInFormat({ "iDate": oAGVStart }, "MMM", false, true) + " " + to.getNumberStringInFormat(oAGVStart.d, 0, true) + "일  -  " + to.getDateInFormat({ "iDate": oAGVEnd }, "MMM", false, true) + " " + to.getNumberStringInFormat(oAGVEnd.d, 0, true) + "일";
				else
                    sHeaderViewLabel = to.getNumberStringInFormat(oAGVEnd.y, 0, true) + ' ' + to.getDateInFormat({ "iDate": oAGVStart }, "MMM", false, true) + " " + to.getNumberStringInFormat(oAGVStart.d, 0, true) + ",  " + to.getNumberStringInFormat(oAGVStart.y, 0, true) + "일  -  " + to.getDateInFormat({"iDate": oAGVEnd}, "MMM", false, true) + " " + to.getNumberStringInFormat(oAGVEnd.d, 0, true) + "일";
			}
			else
                sHeaderViewLabel = to.getNumberStringInFormat(oAGVEnd.y, 0, true) + " " + to.getDateInFormat({"iDate": oAGVStart}, "DDDD", false, true) + " " + to.getDateInFormat({"iDate": oAGVEnd}, "MMMM", false, true) + " " + to.getNumberStringInFormat(oAGVEnd.d, 0, true);
		}
